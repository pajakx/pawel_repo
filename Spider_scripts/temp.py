...
Data_rawEEG = mne.io.read_raw_bdf(r'C:\Users\user\Desktop\FOCUS\2_18_10_1995MAKAAL60.bdf', preload=True)
...
mne.viz.plot_compare_evokeds(dict(neutral=trig_neutral_frontal, buzz=trig_buzz_frontal), legend=True, title=None)

#Plot the different conditions
ylength = dict(eeg=[-12, 12])
pick = all_evokeds_parietal[1].ch_names.index('POz') # just to choose an electrode, index doesn't matter
mne.viz.plot_compare_evokeds(dict(neutral=trig_neutral_parietal, buzz=trig_buzz_parietal), legend=True, title=None)

#Plot the different conditions
ylength = dict(eeg=[-12, 12])
pick = all_evokeds_targets[2].ch_names.index('POz') # just to choose an electrode, index doesn't matter
mne.viz.plot_compare_evokeds(dict(neutral=trig_number_neutral, buzz=trig_number_buzz, nodis=trig_number_nodis), legend=True, title=None)