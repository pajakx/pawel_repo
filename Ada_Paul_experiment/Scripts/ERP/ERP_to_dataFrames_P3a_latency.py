# -*- coding: utf-8 -*-
"""
Created on Wed Feb 19 17:23:37 2020
 _________________                                     
|  ___| ___ \ ___ \                                    
| |__ | |_/ / |_/ /                                    
|  __||    /|  __/                                     
...
for participant in participants:
    ...
    data_ready_sound_buzz = data_buzz.idxmax()
    data_ready_sound_neutral = data_neutral.idxmax()
    
    #Convert series to dataframe, rename the amplitude column, 
    ##add a column with participant info,reindex to 0
    
    #pick the max amplitude in given time window
    data_ready_buzz = data_ready_sound_buzz.idxmax()
    data_ready_neutral = data_ready_sound_neutral.idxmax()
    ...
    df_buzz = df_buzz.rename(columns={0: "Sound_Block_Buzz_latency_Cz"})
    df_buzz['participant'] = participantId 
    df_buzz = df_buzz.rename(index={'Cz':0})
    #for sound neutral
    df_neutral = pandas.DataFrame(data_ready_sound_neutral)
    df_neutral = df_neutral.rename(columns={0: "Sound_Block_Neutral_latency_Cz"})
...
data_latency_max_amplitudes_sounds = pandas.concat(master_list,ignore_index=True)

#Save dataframe to csv file

data_latency_max_amplitudes_sounds.to_csv(r'C:\Users\user\Desktop\FOCUS\data_amplitudes\latency_max_amp_sounds.csv', index = None, header=True)
